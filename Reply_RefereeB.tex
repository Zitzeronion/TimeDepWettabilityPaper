...
\begin{figure}
    \centering
    \includegraphics [width=0.75\textwidth]{small_slip_two_Gammas.pdf}
...
%The dimensions of the substrate should be roughly ten times the maximal pattern wavelength %($\approx 40\,\mu \text{m}$) to account for the periodicity.
%An ideal control of the external stimuli (light) would be a digital multimirror device (DMD).
...
%Using low intensity UV light $365\,\text{nm}$ and blue light %$436$nm~\cite{IchimuraEtAl_Science2000}, we assume that the radiation heat is negligible.
...
We have added the following footnote after the introduction of the contact angle pattern (Eq.(3) in the revised
manuscript) commenting on this aspect:\\
\\
\textcolor{red}{A laboratory experiment for the here mentioned approach consists of a light switchable substrate with a thin film cast on it~\cite{becker2003complex, IchimuraEtAl_Science2000}. 
